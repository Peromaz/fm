...
int main(int argc, char **argv){
	...
	wdraw_menu(curr_dir_win, curr_dir_menu, 3, 1, true, total_menu_rows); 

	draw_panel(next_dir_win, window_width, "NEXT");
	wdraw_menu(next_dir_win, next_dir_menu, 3, 1, false, total_menu_rows); 

	int ch; // This is the character input from the user
	/* Main input loop */
	while((ch = wgetch(curr_dir_win)) != 'q'){ //'q' to quit. 
		/* Gets menu input */
		menu_driver(curr_dir_menu, ch);
